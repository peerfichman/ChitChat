...
const SetExperiment = () => {
    ...
    const setAgent = (id, agentObj ) => {
        const updatedAgents = AIAgents.map((agent) =>{
            if (agent.id === id){
                if('name' in agentObj) return {...agent, name: agentObj.name};
                if('sentiment' in agentObj) return {...agent, sentiment: agentObj.sentiment};
            }
            return agent;
        });
                ...
                <Form onSubmit={(e) => newExperiment(e)}>
                    <Stack direction="vertical" gap={3}>
                        ...
                        <Form.Group className="mb-3">
                            ...
                            <Form.Label> Experiment Starting prompt: </Form.Label>
                            <Form.Control
                                type="text"
                                value={expPrompt}
                        ...
                        <Form.Group className="mb-3">
                            <Stack direction="horizontal" gap={3}>
                                <Form.Label> Number of Intelligent Agents: </Form.Label>

                                <Button onClick={incAgents} className="btn-sm" variant="secondary">+</Button>
                                <Form.Label>{numAIAgents}</Form.Label>
                                <Button onClick={decAgents} className="btn-sm" variant="secondary">-</Button>
                            </Stack>
                        </Form.Group>
                    ...
                    <Stack direction="horizontal" gap={3}>
                        {AIAgents?.map((agent) => {
                            return <AgentCard agent={agent} setAgent={setAgent} />
                        })}
                    </Stack>
